    ...
    return (
        <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "6px", color: "white" }}>
            ...
            <div style={{ fontSize: "13px" }}>
                🧱 <span style={{ color: "#64ffda" }}>{score}</span>/{totalBricks}
            ...
            <div onMouseMove={handleMove} onTouchMove={handleMove} style={{
                ...
                background: "linear-gradient(180deg, #0a0a2e, #0a0a1e)",
                ...
                {bricks.filter((b) => b.alive).map((b) => (
                    <div key={`${b.x}-${b.y}`} style={{
                        ...
                        borderRadius: "3px",
                        boxShadow: `0 1px 4px ${b.color}33, inset 0 1px 2px rgba(255,255,255,0.2)`,
                    }} />
                ))}
                {/* Particles */}
                {particles.map((p) => (
                    <div key={p.id} style={{
                        ...
                        animation: "brickExplode 0.4s ease forwards",
                    }} />
                ))}
                {/* Ball */}
                <div style={{
                    position: "absolute", left: ball.x - BALL_R, top: ball.y - BALL_R,
                    width: BALL_R * 2, height: BALL_R * 2, borderRadius: "50%",
                    background: "white",
                    boxShadow: "0 0 10px white, 0 0 20px rgba(255,255,255,0.3)",
                }} />
                {/* Paddle */}
                <div style={{
                    position: "absolute", left: paddleX, top: H - 20,
