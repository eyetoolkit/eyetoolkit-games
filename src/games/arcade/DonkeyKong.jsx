...
const DonkeyKong = ({ onComplete }) => {
    ...
    const stateRef = useRef({
        px: 160, py: 396, vx: 0, vy: 0,
        onGround: true, climbing: false,
        barrels: [], score: 0, lives: 3,
        gameOver: false, won: false, frameCount: 0,
    });
    ...
    const spawnBarrel = useCallback(() => ({
        x: 100 + Math.random() * 120, y: 60,
        vx: (Math.random() > 0.5 ? 1 : -1) * (1.5 + Math.random()),
        vy: 0, onPlatform: false, active: true,
    }), []);
    ...
    useEffect(() => {
        ...
        const loop = () => {
            ...
            // Check ladder
            const onLadder = LADDERS.find(l =>
                Math.abs(s.px - l.x) < 15 && s.py >= l.y - l.h && s.py <= l.y + 10
            );
            ...
            for (const p of PLATFORMS) {
                if (s.py + PLAYER_H >= p.y && s.py + PLAYER_H <= p.y + PLAT_H + 4 &&
                    s.px + PLAYER_W / 2 > p.x && s.px - PLAYER_W / 2 < p.x + p.w && s.vy >= 0) {
                    s.py = p.y - PLAYER_H;
                    s.vy = 0;
                    s.onGround = true;
                }
            ...
            for (const b of s.barrels) {
                ...
                b.y += b.vy;
                for (const p of PLATFORMS) {
                    if (b.y + 12 >= p.y && b.y + 12 <= p.y + PLAT_H + 4 && b.x > p.x && b.x < p.x + p.w && b.vy >= 0) {
                        b.y = p.y - 12; b.vy = 0; b.onPlatform = true;
                    }
                }
                if (b.x < 0 || b.x > CANVAS_W || b.y > CANVAS_H) b.active = false;
                // Collision with player
                if (Math.abs(b.x - s.px) < 18 && Math.abs(b.y - s.py) < 20) {
                    s.lives--;
                    b.active = false;
                    s.px = 160; s.py = 396;
                }
            ...
            if (s.py < 80) {
                ...
                setTimeout(() => onComplete(Math.min(100, 60 + s.lives * 10)), 500);
                return;
            }
            ...
            if (s.lives <= 0) {
                ...
                setTimeout(() => onComplete(Math.max(20, Math.min(100, s.score / 10))), 500);
                return;
            }
            ...
            for (const p of PLATFORMS) ctx.fillRect(p.x, p.y, p.w, PLAT_H);
            ...
            for (const l of LADDERS) {
                ...
                ctx.beginPath(); ctx.moveTo(l.x + 8, l.y); ctx.lineTo(l.x + 8, l.y - l.h); ctx.stroke();
                for (let ry = l.y; ry > l.y - l.h; ry -= 12) {
                    ctx.beginPath(); ctx.moveTo(l.x - 8, ry); ctx.lineTo(l.x + 8, ry); ctx.stroke();
                }
            }
            ...
            for (const b of s.barrels) {
                ...
                ctx.fillStyle = "#000"; ctx.fillText("B", b.x - 4, b.y + 4); ctx.fillStyle = "#FF6B6B";
            }

            // Player
            ctx.fillStyle = "#4D96FF";
            ctx.fillRect(s.px - PLAYER_W / 2, s.py, PLAYER_W, PLAYER_H);
            ctx.fillStyle = "#FFD93D";
            ctx.beginPath(); ctx.arc(s.px, s.py - 4, 8, 0, Math.PI * 2); ctx.fill();
